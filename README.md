...
## Creating an Interpreter and Script
All `zlisp` code runs from a `Script` struct, which holds a local environment variables and manages its memory. A script is created
from an `Interpreter` struct. The interpreter holds global variables and functions which all scripts may use. Once the interpreter
is deinitialized, all scripts it spawned are freed.

To run `zlisp` code, first create an `Interpreter`.
```zig
var interpreter = Interpreter.init(allocator);
defer interpreter.deinit();
```
You can load variables/functions defined in a `zlisp` file into the interpreter environment using
```zig
try interpreter.loadFile("my_file.lisp");
```
You can get variables defined in the global interpreter environment using the `get` function:
```zig
/// Directly returns the given lisp variable from the environment.
pub fn get(self: Interpreter, varname: []const u8) ?LispType
```
Then, you can create a `Script`:
```zlisp
var script = interpreter.createScript();
```
You may also load a file from a script. All variables/functions in the file will be local to the script, that is, other scripts
will not be able to access.
```zig
try script.loadFile("my_file.lisp");
```
To run `zlisp` code, the following helper functions exist:
```zig
/// Runs the given lisp expression and returns the result value.
/// The value is freed once `re`, `rep` or `reOwned` is called again.
pub fn re(self: *Script, text: []const u8) !LispType

/// Runs the given lisp expression and returns the result value. Value is owned by the caller.
pub fn reOwned(self: *Script, allocator: std.mem.Allocator, text: []const u8) !LispType

/// Directly returns the given lisp variable from the environment.
pub fn get(self: Script, varname: []const u8) ?LispType
...
pub fn rep(self: *Script, text: []const u8) ![]const u8
```
...
## Builtins
See [BUILTINS.md](BUILTINS.md) for a list of `zlisp`'s builtin functions.

## Memory
All `zlisp` variables live on the stack, including lists, vectors and dictionaries. Dynamic memory allocation and management must be done on
`zig`'s side.
