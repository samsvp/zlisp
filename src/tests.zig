...
test "math ops" {
    ...
    var file = try std.fs.cwd().openFile("src/test-files/math.lisp", .{});
    defer file.close();
    ...
    while (it.next()) |line| {
        ...
        const ret = interpreter.re(line) catch {
            std.debug.print("ERR line {}: {s}\n", .{ i, interpreter.err_ctx.buffer.items });
        ...
        std.testing.expect(ret.eql(LispType.lisp_true)) catch {
            ...
            std.debug.print("ERR line {}: return value {s}\n", .{ i, str });
            try std.testing.expect(false);
        };
    }
}
